...
#Filtering out the healthy individuals
healthy <- meta %>%
  filter(meta$Disease == "Control")
nrow(healthy)

#pulling out the samples who are not retrieved from Parkinsons patients, and who have lived on a farm
healthyfarm <- meta %>%
  filter(meta$Lived_on_farm == "Yes" & meta$Disease == "Control") 

                 # no constipation for "healthyfarm

#generating a new otu table with only the samples from non-PD individuals who lived on a farm
otu_hf <- otu %>% 
  select(all_of(c(healthyfarm$`#SampleID`))) %>%
...
rarecurve(t(as.data.frame(otu_table(phylobj))), cex=0.01)

#generating new phyloseq object which has been rarefied, cutoff decided from rarefaction curve above
phylobj_raref <- rarefy_even_depth(phylobj, rngseed = 1, sample.size = 5500)
...
#Loading data
load("phyloseqobject_mouse.RData")
load("phyloseqobject_mouse_rarefied.RData")


distanceMethodList


#Producing unifrac PCoA plot
unif <- distance(phylobj_raref, method="unifrac")
pcoab <- ordinate(phylobj_raref, method="PCoA", distance=unif)

ggpcoa <- plot_ordination(phylobj_raref, pcoab)+
  ggtitle("unifrac PCoA Analysis of non-PD, yes-farm ")

ggpcoa #viewing the PCoA plot
